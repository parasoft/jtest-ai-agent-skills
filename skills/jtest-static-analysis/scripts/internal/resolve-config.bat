@echo off
rem =============================================================================
rem resolve-config.bat  —  Load, parse, and validate all Jtest Static Analysis settings
rem
...
rem   JTEST_HOME, ANALYZED_PROJECT_PATH, JTEST_STATIC_CONFIGURATION, JTEST_COMMIT_FIXES,
...
rem   JTEST_STATIC_SCRIPT_DIR, JTEST_RESOURCE
rem =============================================================================
...
for /f "usebackq tokens=1,* delims==" %%A in ("%JTEST_SKILLS_CONFIG%") do (
    ...
    if not "!_cfgkey:~0,1!"=="#" (
        ...
        if "!_cfgkey!"=="JTEST_STATIC_CONFIGURATION" if not defined JTEST_STATIC_CONFIGURATION endlocal & set "JTEST_STATIC_CONFIGURATION=%%B" & setlocal enabledelayedexpansion
        ...
        if "!_cfgkey!"=="JTEST_STATIC_SCRIPT_DIR" if not defined JTEST_STATIC_SCRIPT_DIR endlocal & set "JTEST_STATIC_SCRIPT_DIR=%%B" & setlocal enabledelayedexpansion
...
rem ---- JTEST_STATIC_CONFIGURATION ----------------------------------------------
if not defined JTEST_STATIC_CONFIGURATION set "JTEST_STATIC_CONFIGURATION=builtin://Recommended Rules"
if "%JTEST_STATIC_CONFIGURATION%"=="" set "JTEST_STATIC_CONFIGURATION=builtin://Recommended Rules"
...
rem ---- JTEST_STATIC_NO_OF_MAX_FIXES --------------------------------------------------
if not defined JTEST_STATIC_NO_OF_MAX_FIXES set "JTEST_STATIC_NO_OF_MAX_FIXES=10"
if "%JTEST_STATIC_NO_OF_MAX_FIXES%"=="" set "JTEST_STATIC_NO_OF_MAX_FIXES=10"

rem ---- JTEST_STATIC_SCRIPT_DIR ------------------------------------------------------------
set "_RC_JTEST_STATIC_SCRIPT_DIR_SOURCE=(default)"
if not defined JTEST_STATIC_SCRIPT_DIR (
    set "JTEST_STATIC_SCRIPT_DIR=%_RC_SKILL_DIR%\scripts"
) else if "%JTEST_STATIC_SCRIPT_DIR%"=="" (
    set "JTEST_STATIC_SCRIPT_DIR=%_RC_SKILL_DIR%\scripts"
) else (
    set "_RC_JTEST_STATIC_SCRIPT_DIR_SOURCE=(override)"
)

if not exist "%JTEST_STATIC_SCRIPT_DIR%\" (
    echo ERROR: JTEST_STATIC_SCRIPT_DIR points to a directory that does not exist: %JTEST_STATIC_SCRIPT_DIR%. Verify the path and retry.
    exit /b 1
)

rem Validate required scripts exist (prefer .bat, then .ps1)
set "_RC_BV_OK=0"
if exist "%JTEST_STATIC_SCRIPT_DIR%\build-verify\build-verify.bat" set "_RC_BV_OK=1"
if exist "%JTEST_STATIC_SCRIPT_DIR%\build-verify\build-verify.ps1" set "_RC_BV_OK=1"
if "%_RC_BV_OK%"=="0" (
    echo ERROR: JTEST_STATIC_SCRIPT_DIR=%JTEST_STATIC_SCRIPT_DIR% is missing required script build-verify.bat ^(or .ps1^). Provide both build-verify and jtest-analyze scripts and retry.
    exit /b 1
)

set "_RC_JA_OK=0"
if exist "%JTEST_STATIC_SCRIPT_DIR%\jtest-analyze\jtest-analyze.bat" set "_RC_JA_OK=1"
if exist "%JTEST_STATIC_SCRIPT_DIR%\jtest-analyze\jtest-analyze.ps1" set "_RC_JA_OK=1"
if "%_RC_JA_OK%"=="0" (
    echo ERROR: JTEST_STATIC_SCRIPT_DIR=%JTEST_STATIC_SCRIPT_DIR% is missing required script jtest-analyze.bat ^(or .ps1^). Provide both build-verify and jtest-analyze scripts and retry.
    exit /b 1
)
...
echo   JTEST_STATIC_CONFIGURATION = %JTEST_STATIC_CONFIGURATION%
...
echo   JTEST_STATIC_NO_OF_MAX_FIXES    = %JTEST_STATIC_NO_OF_MAX_FIXES%
if "%JTEST_REFERENCE_BRANCH%"=="" (
    echo   JTEST_REFERENCE_BRANCH      = (not set^)
) else (
    echo   JTEST_REFERENCE_BRANCH      = %JTEST_REFERENCE_BRANCH%
)
echo   JTEST_STATIC_SCRIPT_DIR               = %JTEST_STATIC_SCRIPT_DIR% %_RC_JTEST_STATIC_SCRIPT_DIR_SOURCE%
if "%JTEST_RESOURCE%"=="" (
...
rem ===== Write resolved configuration to resolved.config ======================
(
    echo Resolved configuration:
    if defined JTEST_SKILLS_CONFIG (
        echo   JTEST_SKILLS_CONFIG    = %JTEST_SKILLS_CONFIG%
...
set "_RC_SKILL_DIR="
set "_RC_JTEST_STATIC_SCRIPT_DIR_SOURCE="
set "_RC_BV_OK="
set "_RC_JA_OK="
set "_RC_HAS_TARGET="
