#!/usr/bin/env bash
# =============================================================================
# resolve-config.sh  —  Load, parse, and validate all Jtest Static Analysis settings
#
...
#   JTEST_STATIC_SCRIPT_DIR, JTEST_RESOURCE  (set to empty string when not applicable)
# =============================================================================
...
export JTEST_STATIC_CONFIGURATION="${JTEST_STATIC_CONFIGURATION:-builtin://Recommended Rules}"

# ---- JTEST_COMMIT_FIXES -----------------------------------------------------
export JTEST_COMMIT_FIXES="${JTEST_COMMIT_FIXES:-false}"

# ---- JTEST_STATIC_FILTER_RULE ------------------------------------------------------
export JTEST_STATIC_FILTER_RULE="${JTEST_STATIC_FILTER_RULE:-}"
...
export JTEST_STATIC_NO_OF_MAX_FIXES="${JTEST_STATIC_NO_OF_MAX_FIXES:-10}"

# ---- JTEST_STATIC_SCRIPT_DIR --------------------------------------------------------------
_JTEST_STATIC_SCRIPT_DIR_SOURCE="(default)"
if [ -z "${JTEST_STATIC_SCRIPT_DIR:-}" ]; then
    JTEST_STATIC_SCRIPT_DIR="${_SKILL_DIR}/scripts"
else
    _JTEST_STATIC_SCRIPT_DIR_SOURCE="(override)"
fi
[ -d "${JTEST_STATIC_SCRIPT_DIR}" ] || _die "JTEST_STATIC_SCRIPT_DIR points to a directory that does not exist: ${JTEST_STATIC_SCRIPT_DIR}. Verify the path and retry."

# Validate required scripts exist
_found_build_verify=0
for _name in build-verify/build-verify.sh; do
    [ -f "${JTEST_STATIC_SCRIPT_DIR}/${_name}" ] && _found_build_verify=1
done
[ "${_found_build_verify}" -eq 1 ] || _die "JTEST_STATIC_SCRIPT_DIR=${JTEST_STATIC_SCRIPT_DIR} is missing required script build-verify.sh. Provide both build-verify and jtest-analyze scripts and retry."

_found_jtest_analyze=0
for _name in jtest-analyze/jtest-analyze.sh; do
    [ -f "${JTEST_STATIC_SCRIPT_DIR}/${_name}" ] && _found_jtest_analyze=1
done
[ "${_found_jtest_analyze}" -eq 1 ] || _die "JTEST_STATIC_SCRIPT_DIR=${JTEST_STATIC_SCRIPT_DIR} is missing required script jtest-analyze.sh. Provide both build-verify and jtest-analyze scripts and retry."

export JTEST_STATIC_SCRIPT_DIR
...
Resolved configuration:
  ...
  JTEST_STATIC_CONFIGURATION = ${JTEST_STATIC_CONFIGURATION}
  JTEST_COMMIT_FIXES       = ${JTEST_COMMIT_FIXES}
  JTEST_STATIC_FILTER_RULE        = ${JTEST_STATIC_FILTER_RULE:-(not set)}
  ...
  JTEST_STATIC_NO_OF_MAX_FIXES    = ${JTEST_STATIC_NO_OF_MAX_FIXES}
  JTEST_REFERENCE_BRANCH      = ${JTEST_REFERENCE_BRANCH:-(not set)}
  JTEST_STATIC_SCRIPT_DIR               = ${JTEST_STATIC_SCRIPT_DIR} ${_JTEST_STATIC_SCRIPT_DIR_SOURCE}
